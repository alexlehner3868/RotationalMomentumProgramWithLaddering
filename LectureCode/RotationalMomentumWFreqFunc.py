...
"""
This program assumes bonds and stocks are inversely correlated.
But this is not always true, 
the inverse correlation depends on inflation (e.g. 5-Year Breakeven Inflation Rate = T5YIE https://fred.stlouisfed.org/series/T5YIE)
If T5YIE < 2.5%, the 90-day rolling correlation between stocks e.g. SPX and bonds e.g. TLT is negative.
If T5YIE > 2.5%, the 90-day rolling correlation between stocks e.g. SPX and bonds e.g. TLT is positive.
For a rotational strategy, 
if T5YIE > 2.5% the safe asset must be as short a bond as possible: SGOV, otherwise, it can by SHY.
IF the FED is rising interest rates because T5YIE > 2.5%, an option is to include UUP in the investment list.
if T5YIE < 2.5% the safe asset can be a long bond: TLT.
You need not invest in UUP.
"""
...
#dfP = pd.read_csv('SPY.SHY.csv', parse_dates=['Date'])
#dfAP = pd.read_csv('SPY.SHY.AP.csv', parse_dates=['Date'])
#dfP = pd.read_csv('UUP.XLB.XLE.XLF.XLI.XLK.XLP.XLU.XLV.XLY.XRT.SHY.csv', parse_dates=['Date'])
#dfAP = pd.read_csv('UUP.XLB.XLE.XLF.XLI.XLK.XLP.XLU.XLV.XLY.XRT.SHY.AP.csv', parse_dates=['Date'])
dfP = pd.read_csv('BIL.TIP.IEI.IEF.TLH.TLT.SHY.csv', parse_dates=['Date'])
dfAP = pd.read_csv('BIL.TIP.IEI.IEF.TLH.TLT.SHY.AP.csv', parse_dates=['Date'])
#dfP = pd.read_csv('BIL.TIP.IEI.csv', parse_dates=['Date'])
#dfAP = pd.read_csv('BIL.TIP.IEI.AP.csv', parse_dates=['Date'])

dfP = dfP.fillna(method='ffill')  # Forward-fill and assign back
dfAP = dfAP.fillna(method='ffill')  # Forward-fill and assign back
...
logReturns = 0
Aperiods = 40 #Lookback 20 Default
Frequency = "11W-FRI" #fridays and thursdays are the best trading days, 18W-FRI Default
ShortTermWeight = .4
LongTermWeight = .3
ShortTermVolatilityWeight = .3
Delay = 1


#experimental cash filter
CashFilter = 0
MAperiods = 200 #200 Moving Average condition for DIA (Dow Jones) or for selected ETF
StandsForCash = "SHY" 
Zboundary = -1.5 
Zperiods = 200  
...
#################################################################################################################
#Weights of the varous ranks (do not add up to one) #############################################################
...
for row in range(rows):
    ...
    if CashFilter == 1:
        if (dfP[dfP.columns[max_arr_column]][row] >= dfMA[dfMA.columns[max_arr_column]][row]): #"200MA" condition for cash filter applies to selected ETF
        #if (dfZ[dfZ.columns[max_arr_column]][row] > Zboundary):  #"200MA" condition for cash filter applies to Dow Jones (requires the inclusion of DIA ETF (Dow Jones) in the list)
